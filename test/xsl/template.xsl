...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/*">
	<html>
		<xsl:variable name="title" select="'Templating'" />
		<head>
			...
			<table>
				...
				<tbody>
					<tr>
						...
						<td>Application self-detects its root and passes it to template</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							<xsl:call-template name="html-link">
								<xsl:with-param name="value" select="'Article #3'" />
								<xsl:with-param name="href" select="'/article/3'" />
							</xsl:call-template>
						</td>
						<td>Automatically prefixed by base path, generated by XSLT</td>
					</tr>
					<tr>
						<td>External link with rich content</td>
						<td>
							<xsl:call-template name="html-link">
								<xsl:with-param name="value"><img src="http://www.google.com/images/logo_sm.gif"/></xsl:with-param>
								<xsl:with-param name="href" select="'http://www.google.com/'" />
							</xsl:call-template>
						</td>
						<td>No prefix used, generated by XSLT</td>
					</tr>
					<tr>
